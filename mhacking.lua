	...
	while true do
		...
		if showHelp then
			if helpTimer > GetGameTimer() then
				showHelpText("Naviguez avec ~y~Z,Q,S,D~s~ et confirmez avec ~y~ESPACE~s~ pour le bloc de code de gauche.")
			elseif helpTimer > GetGameTimer()-helpCycle then
				showHelpText("Utilisez les ~y~touches directionnelles~s~ et ~y~ENTER~s~ pour le bloc de code de droite")
			else
